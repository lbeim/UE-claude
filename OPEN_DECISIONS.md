...
**Fortsetzung 2026-06-01 (Plan `delightful-conjuring-pearl`, Operator-approved) — Namen-Keying + Solid-Mirror + Farbraum:**
Pivot weg von stateId-Zeilen (heute `Row=round(FloatArgs[0])`, **kein** Namen-Gedächtnis → ID-Reshuffle in React scrambelt alle Zeilen) hin zu **Namen als einzigem Schlüssel**:
- **Namen→Zeilen-Register** in der Bridge (`TMap<FName,int32>` UPROPERTY am Router, level-gespeichert) — **append-only + Tombstone**: neuer Name → nächste freie Zeile, nie umsortiert; in React entfernte Farbe behält ihre (verwaiste) Zeile. **Reorder/Rename nur Claude-assistiert.** → full-send idempotent/additiv. `stateId`/`oscIndex` raus (inkl. React-Badge).
- **Base-Name-Keying**: `/col/<name>/curve|/rgb|<leer>` → erstes Segment = Basisname → Kurven- **und** Solid-Form derselben Farbe = **dieselbe** Zeile.
- **Solid-Mirror**: feste Farben zusätzlich als **konstante volle Zeile** in DENSELBEN Atlas (Einbahnstraße: Solid→Atlas automatisch, Kurve→Einzel-RGB **nie** automatisch); MPC-Direkt-Binding bleibt zusätzlich. Zweites 1×1-RT **verworfen** (~200 KB < Komplexität; Uniformität „Zeile=Farbe" wiegt mehr).
- **Farbraum = durchgängig linear** (Operator-Call an Claude delegiert): React **zeigt** sRGB, **sendet** linear-HDR-Float; Bridge speichert **roh** (kein `FromSRGBColor` auf `/col/*`; MPC+Atlas **gleich** interpretieren). Grund: sRGB=0..1 killt HDR, Kurven sind schon linear (bis 10). **Szenen-Anzeige-Match = Tonemap/Exposure (ACES), szenenweit** → separater Show-Hebel (unlit-Material), nicht Teil dieses Builds. **16-bit/Float senden** (bandingfrei). **Controller-Checkpoint (Operator klärt):** linear-HDR-fähig? Ja=sauber; Nein (8-bit sRGB hart)=Fallback sRGB-Decode **ohne** HDR>1.
- **RT-Settings engine-verifiziert (UE 5.8)**: `RTF_RGBA16f`=HDR-Boden (kein 3-Kanal-Float-RT; `RGB10A2` UNORM 0..1; Alpha nicht abwählbar, im Atlas ungenutzt = Position auf X-Achse). Float-RT immer linear. Filter `TF_Bilinear`+Clamp, **V=`(row+0.5)/H`** → kein Zeilen-Bleed. RTs unkomprimiert. Atlas großzügig fix (256×256 ≈512 KB), `AtlasHeight`-Param fix=SizeY (kein Resync).
- **Status**: §D (React: IDs raus, Stream sauber + bei Refresh bestückt) **durch**, §E (Wartung Claude-assistiert) bestätigt. **§A+§B gebaut grün** 2026-06-01 (beide DLLs via `bridge_rebuild`; Eye-Test offen). Umsetzung in `OSCBridgeRouter.{h,cpp}`: Pattern-Matching auf **inneres Wildcard** erweitert (`/col/*/rgb` möglich, trailing `/*` bleibt back-compat); Register `ColorRowRegistry`+`NextFreeRow`+`GetOrAssignColorRow` (VisibleAnywhere, level-gespeichert); `ApplyCurveBinding` branched per Adress-Suffix (`/curve`=Gradient, sonst=konstante Zeile/Solid-Mirror), Zeile aus `OSCBridge_BaseColorKey` (Adresse minus `/curve|/rgb`) → Register, **format-tolerant** gg. legacy führendes `stateId` (Länge mod 4), Dirty-Check via `LastAtlasRowByKey`; `CreateOrResizeGradientAtlas` setzt `TF_Bilinear`+`TA_Clamp` & Default-Height 256. Solid→Atlas läuft über die `/col/*`-Curve-Binding; Solid→MPC bleibt separates Vector-Binding (Pattern `/col/*/rgb`). §C Material/Niagara offen. Memory [[reference-osc-color-pipeline-linear]] + [[reference-ue-runtime-rendertarget-write]].
...
**Fortschritt 2026-06-02 — BASS-Slice implementiert + grün verifiziert:** Der kurvengetriebene Pivot ist für **BASS** umgesetzt und läuft grün. **Klarstellung zur Material-Architektur:** `M_RadialPulse` ist **EINE selbst-zyklende radiale Welle** (ein Custom-HLSL-Node, `radius = Speed*(Time − floor(Time/Period)*Period)`) — **KEINE 8-Slot-Kaskade** (die frühere Doku/Memory war falsch, korrigiert in [[project-pulsekit-radial-pulse]]). **Mechanismus:** `UPulseTriggerComponent::DoTick` liest **kontinuierlich** den OSC_MPC-Scalar `BASS` (= der geglättete `/mod/mod/BASS_xf`, als MPC-Param-Name `BASS` geliefert — siehe [[reference-osc-mod-channel-map]]), normalisiert (`BassNormMin/Max` + `BassDriveExponent` → 0..1 drive) und schreibt **jeden Tick** `PulseGain/Speed/Bounds/Width = Base + Amount*drive` auf die MIDs → die EINE Welle **atmet** mit dem Bass (kein binäres Ploppen). Neue Tunables unter Kategorie `Pulse|Curve`; Modulation läuft VOR dem `bAutoFirePreview`-Early-Return (Preview + reaktiv). **Shader-Graph-Injektion via MCP war infeasibel** (Custom-Node-Input-Pins nicht lesbar) → Modulation lebt im C++-`DoTick`. **Getunte Defaults:** NormMax **0.6**, Exp **1.4** (+ Gain 0.4/3.5, Speed 300/400, Bounds 1200/700, Width 25/40). Files: `Plugins/PulseKit/Source/PulseKit/{Public/PulseTriggerComponent.h, Private/PulseTriggerComponent.cpp}`. **Persistenz-Falle:** die `PulseTrigger`-Komponente wird vom BP-Construction-Script bei jedem Reconstruct neu erzeugt → Instanz-`set_properties`+Save persistiert NICHT, nur C++-Defaults+Rebuild ([[reference-bp-cs-recreated-component-no-instance-persist]]). **Deferred:** low/mid/high + die weiteren Report-Kanäle (`channel-map.md`) noch nicht angeschlossen. (Alte binäre Threshold-Spawn-Logik bleibt im Code, gegen das Single-Pulse-Material inert.)
